package ae.zerotone.gateway.assist.config;

import ae.zerotone.gateway.assist.application.GatewayApplication;
...
public class GatewayAutoConfig {
  ...
  public RedisConnectionFactory redisConnectionFactory(
      GatewayServiceProperties properties, GatewayCenterService gatewayCenterService) {
    // 1. 拉取注册中心的 Redis 配置信息
    Map<String, String> redisConfig =
        gatewayCenterService.queryRedisConfig(properties.getAddress());
    // 2. 构建 Redis 服务
  ...
  public RedisMessageListenerContainer container(
    ...
    //  container.addMessageListener(msgAgreementListenerAdapter, new
    // PatternTopic("api-gateway-g4"));
    container.addMessageListener(
        msgAgreementListenerAdapter, new PatternTopic(properties.getGatewayId()));
    return container;
  }
  ...
  @Bean
  public GatewayApplication gatewayApplication(
      GatewayServiceProperties properties,
      GatewayCenterService registerGatewayService,
      ae.zerotone.gateway.core.session.Configuration configuration,
      Channel gatewaySocketServerChannel) {
    return new GatewayApplication(
        properties, registerGatewayService, configuration, gatewaySocketServerChannel);
  }

  /** 创建网关配置对象；Configuration 是用于贯穿整个网关核心通信服务的。 */
  @Bean
  public ae.zerotone.gateway.core.session.Configuration gatewayCoreConfiguration(
      GatewayServiceProperties properties) {
    ae.zerotone.gateway.core.session.Configuration configuration =
        new ae.zerotone.gateway.core.session.Configuration();
  ...
  public Channel initGateway(ae.zerotone.gateway.core.session.Configuration configuration)
