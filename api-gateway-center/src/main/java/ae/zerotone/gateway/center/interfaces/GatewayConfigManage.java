...
import ae.zerotone.gateway.center.application.ILoadBalancingService;
...
import ae.zerotone.gateway.center.domain.docker.model.vo.LocationVO;
import ae.zerotone.gateway.center.domain.docker.model.vo.UpstreamVO;
...
import ae.zerotone.gateway.center.infrastructure.common.ResponseCode;
import ae.zerotone.gateway.center.infrastructure.common.Result;
...
/**
 * @description 网关配置管理；服务分组、网关注册、服务关联 1. 查询网关服务配置项信息：/wg/admin/config/queryServerConfig 2.
 *     注册网关服务节点：/wg/admin/config/registerGateway
 */
@RestController
...
public class GatewayConfigManage {
  ...
  @Resource private ILoadBalancingService loadBalancingService;
  ...
  public Result<List<GatewayServerVO>> queryServerConfig() {
    try {
      ...
      return new Result<>(
          ResponseCode.SUCCESS.getCode(), ResponseCode.SUCCESS.getInfo(), gatewayServerVOList);
    } catch (Exception e) {
  ...
  public Result<List<GatewayServerDetailVO>> queryServerDetailConfig() {
    try {
      ...
      List<GatewayServerDetailVO> gatewayServerVOList =
          configManageService.queryGatewayServerDetailList();
      return new Result<>(
          ResponseCode.SUCCESS.getCode(), ResponseCode.SUCCESS.getInfo(), gatewayServerVOList);
    } catch (Exception e) {
  ...
  public Result<List<GatewayDistributionVO>> queryGatewayDistributionList() {
    try {
      ...
      List<GatewayDistributionVO> gatewayServerVOList =
          configManageService.queryGatewayDistributionList();
      return new Result<>(
          ResponseCode.SUCCESS.getCode(), ResponseCode.SUCCESS.getInfo(), gatewayServerVOList);
    } catch (Exception e) {
  ...
  public Result<Boolean> registerGatewayServerNode(
    ...
    try {
      logger.info(
          "注册网关服务节点 gatewayId：{} gatewayName：{} gatewayAddress：{}",
          gatewayId,
          gatewayName,
          gatewayAddress);
      // 1. 注册&更新网关算力信息
      boolean done =
          configManageService.registerGatewayServerNode(
              groupId, gatewayId, gatewayName, gatewayAddress);
      // 2. 读取最新网关算力数据【由于可能来自于多套注册中心，所以从数据库或者Redis中获取，更为准确】
      List<GatewayServerDetailVO> gatewayServerDetailVOList =
          configManageService.queryGatewayServerDetailList();
      // 3. 组装Nginx网关刷新配置信息
      Map<String, List<GatewayServerDetailVO>> gatewayServerDetailMap =
          gatewayServerDetailVOList.stream()
      ...
      for (String name : uniqueGroupIdList) {
        ...
        // }
        List<String> servers =
            gatewayServerDetailMap.get(name).stream()
  ...
  /**
   * 网关算力与系统挂载配置 groupId --1vn--> gatewayId --1vn--> systemId 10001 -> api-gateway-g3 ->
   * api-gateway-test-01-provider 10001 -> api-gateway-g3 -> api-gateway-test-02-provider 10001 ->
   * api-gateway-g4 -> api-gateway-test-03-provider 10001 -> api-gateway-g4 ->
   * api-gateway-test-04-provider 10002 -> api-gateway-g5 -> api-gateway-test-05-provider 10002 ->
   * api-gateway-g5 -> api-gateway-test-06-provider
   */
  @PostMapping(value = "distributionGatewayServerNode", produces = "application/json;charset=utf-8")
  public Result<Boolean> distributionGatewayServerNode(
      @RequestParam String groupId, @RequestParam String gatewayId, @RequestParam String systemId) {
    try {
      ...
      logger.warn(
          "查询应用服务配置项信息失败，唯一索引冲突。groupId：{} gatewayId：{} systemId：{}",
          groupId,
          gatewayId,
          systemId,
          e);
      ...
      logger.error(
          "网关算力与系统挂载配置。groupId：{} gatewayId：{} systemId：{}", groupId, gatewayId, systemId, e);
      return new Result<>(ResponseCode.UN_ERROR.getCode(), e.getMessage(), false);
    }
  }
  ...
  public Result<List<ApplicationSystemVO>> queryApplicationSystemList() {
    try {
      ...
      List<ApplicationSystemVO> gatewayServerVOList =
          configManageService.queryApplicationSystemList();
      return new Result<>(
          ResponseCode.SUCCESS.getCode(), ResponseCode.SUCCESS.getInfo(), gatewayServerVOList);
    } catch (Exception e) {
  ...
  public Result<List<ApplicationInterfaceVO>> queryApplicationInterfaceList() {
    try {
      ...
      List<ApplicationInterfaceVO> gatewayServerVOList =
          configManageService.queryApplicationInterfaceList();
      return new Result<>(
          ResponseCode.SUCCESS.getCode(), ResponseCode.SUCCESS.getInfo(), gatewayServerVOList);
    } catch (Exception e) {
  ...
  public Result<List<ApplicationInterfaceMethodVO>> queryApplicationInterfaceMethodList() {
    try {
      ...
      List<ApplicationInterfaceMethodVO> gatewayServerVOList =
          configManageService.queryApplicationInterfaceMethodList();
      return new Result<>(
          ResponseCode.SUCCESS.getCode(), ResponseCode.SUCCESS.getInfo(), gatewayServerVOList);
    } catch (Exception e) {
  ...
  @PostMapping(
      value = "queryApplicationSystemRichInfo",
      produces = "application/json;charset=utf-8")
  public Result<ApplicationSystemRichInfo> queryApplicationSystemRichInfo(
      @RequestParam String gatewayId, @RequestParam String systemId) {
    try {
  ...
  public Result<Map<String, String>> queryRedisConfig() {
    try {
      ...
      return new Result<>(
          ResponseCode.SUCCESS.getCode(), ResponseCode.SUCCESS.getInfo(), redisConfig);
    } catch (Exception e) {
